...
In Drupal 8 you can create base themes and subthemes. A base theme provides
some set of predefined styles and functionality that a frontend developer can
base a subtheme off of. Any theme can be reused as a base theme, even if it is
a subtheme of some other theme.

## Core themes

Drupal 8 comes with five preloaded themes, located in `core/themes/`.

1. **Stable**: Whenever a new theme is created, this is the default base theme
if no other base themes are specified. It is a minimalist theme, that only introduces
CSS and Javascript necessary for functionality, allowing the themer to supply
their own styles and scripts for everything else, helping maintain backwards
compatibility for the Drupal 8 life cycle.
...
Choosing a base theme is a big decision in creating a new Drupal theme as it lays the framework for the way you will be building your front end. From a D8 perspective,
...
The reason Drupal 8 provides these two base themes is to appeal to two different groups of developers:

- "Sensible 2/3" developers want sensible markup and default classes to apply CSS with minimal overrides.
- "Clean 1/3" developers want a minimalist theme that doesn't place unnecessary markup and css upon themers.
...
But these are just two of a theoretically infinite number of base themes you can choose from, including a custom base theme that is shared by all of your different projects.
...
Inside each theme it is recommended to have a `css`, `js` and `images` directory.
Additionally you should include the following files.
